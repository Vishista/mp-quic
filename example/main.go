...
func main() {
	...
	www := flag.String("www", "./example/web", "www data")
	tcp := flag.Bool("tcp", false, "also listen on TCP")
	port := flag.String("port", "6121", "port to use")
	sc := flag.String("sc", "rr", "schedulers:rr mrtt")
	ssc := flag.String("ssc", "rr", "stream_schedulers:rr prio sprio")
	...
	var ips []string
	getLocalEthernetAddresses(&ips, *ni)
	var allIPs string
	for _, ip := range ips {
		allIPs = allIPs + "+" + ip + ":" + *port
	}
	fmt.Println("Detected Network Interfaces:", ips)
	...
	for _, b := range bs {
		...
		go func() {
			var err error
			if *tcp {
				err = h2quic.ListenAndServe(ips[0]+":"+*port, certFile, keyFile, nil)
			} else {
				err = h2quic.ListenAndServeQUIC(bCap, certFile, keyFile, nil, *sc, *ssc)
			}
			if err != nil {
				fmt.Println(err)
			}
