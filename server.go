...
type server struct {
	...
	newSession            func(connection connection, v protocol.VersionNumber, connectionID protocol.ConnectionID, sCfg *handshake.ServerConfig, config *Config, scheduler string, streamScheduler string) (packetHandler, <-chan handshakeEvent, error)
	scheduler             string
	streamScheduler       string
	multipath_established bool
}
...
func ListenAddr(addr string, config *Config, scheduler string, streamScheduler string) (Listener, error) {
	...
	return Listen(connections, config, scheduler, streamScheduler)
}

// Listen listens for QUIC connections on a given net.PacketConn.
// The listener is not active until Serve() is called.
func Listen(connections []net.PacketConn, config *Config, scheduler string, streamScheduler string) (Listener, error) {
...
func (s *server) serve(index int) {
	for {
		...
		n, remoteAddr, err := s.connections[index].ReadFrom(data)
		if err != nil {
			s.serverError = err
			close(s.errorChan)
			_ = s.Close()
		...
		data = data[:n]
		if err := s.handlePacket(s.connections[index], remoteAddr, data, index); err != nil {
...
func (s *server) Close() error {
	...
	//just return any error if any occured
	return err
}

// Addr returns the server's network address
func (s *server) Addr() net.Addr {
	return s.connections[0].LocalAddr()
}

func (s *server) handlePacket(pconn net.PacketConn, remoteAddr net.Addr, packet []byte, index int) error {
	rcvTime := time.Now()
	...
	//This answers the announcement and tells the client that the announcement can be used now
	if hdr.AnnouncePath {
		if s.multipath_established {
			return nil
		} else {
			s.multipath_established = true
		}
		announcedAddress := remoteAddr
		if _, err = pconn.WriteTo(composeAnnouncement(hdr.ConnectionID, protocol.PerspectiveServer), announcedAddress); err != nil {
			return err
		}
	}
