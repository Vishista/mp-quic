...
func (h *sentPacketHandler) SendingAllowed(id int) bool {
	congestionLimited := h.bytesInFlight > h.congestion.GetCongestionWindow()
	if congestionLimited {
		utils.Debugf("Congestion limited Path %d: bytes in flight %d, window %d",
			id,
			h.bytesInFlight,
			h.congestion.GetCongestionWindow())
	}
	return !congestionLimited
}

func (h *sentPacketHandler) TrackedSentPackets() int {
	return (len(h.retransmissionQueue) + h.packetHistory.Len())
}
func (h *sentPacketHandler) HasRetransmission() bool {
