...
func (s *stream) Read(p []byte) (int, error) {
	...
	for bytesRead < len(p) {
		...
		s.onData(SchedulerSendData{
			StreamId:       s.streamID,
			Priority:       s.priority,
			DataByteLength: 0,
		}) // so that a possible WINDOW_UPDATE is sent
...
func (s *stream) Write(p []byte) (int, error) {
	...
	s.onData(SchedulerSendData{
		StreamId:       s.streamID,
		Priority:       s.priority,
		DataByteLength: protocol.ByteCount(len(p)),
	})
...
func (s *stream) SetPriority(newPriority protocol.PriorityParam) {
