...
type client struct {
	...
	dialAddr func(hostname string, config *quic.Config, localAddr []string, moreRemoteIps []string) (quic.Session, error)
	config   *quic.Config
	...
	localAddr []string
	moreRemoteAddresses []string
}

var _ http.RoundTripper = &client{}

// newClient creates a new client
func newClient(tlsConfig *tls.Config, hostname string, opts *roundTripperOpts, localAddr []string, moreRemoteAddresses []string) *client {
	return &client{
		...
		localAddr:     localAddr,
		moreRemoteAddresses: moreRemoteAddresses,
	}
}
...
func (c *client) dial() error {
	var err error
	c.session, err = c.dialAddr(c.hostname, c.config, c.localAddr, c.moreRemoteAddresses)
	if err != nil {
		return err
	}
