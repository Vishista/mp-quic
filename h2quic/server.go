...
type streamCreator interface {
	...
	SetStreamPriority(protocol.PriorityParam, protocol.StreamID) error
}
...
func (s *Server) serveImpl(tlsConfig *tls.Config, conn *net.UDPConn) error {
	...
	var err error
	if conn == nil {
		ln, err = quic.ListenAddr(s.Addr, &config, s.scheduler, s.streamScheduler)
	} else {
		ln, err = quic.Listen([]net.PacketConn{conn}, &config, s.scheduler, s.streamScheduler)
	}
	if err != nil {
		s.listenerMutex.Unlock()
		return err
...
func (s *Server) handleRequest(session streamCreator, headerStream quic.Stream, headerStreamMutex *sync.Mutex, hpackDecoder *hpack.Decoder, h2framer *http2.Framer) error {
	...
	h2headersFrame.Priority.StreamDep = 0
	h2headersFrame.Priority.Weight = 1
	if h2headersFrame.HasPriority() {
		if strings.HasSuffix(req.URL.Path, "html") {
			h2headersFrame.Priority.Weight = 255
		}
		if strings.HasSuffix(req.URL.Path, "css") {
			h2headersFrame.Priority.Weight = 199
		}
		if strings.HasSuffix(req.URL.Path, "js") {
			h2headersFrame.Priority.Weight = 155
		}
		if strings.HasSuffix(req.URL.Path, "png") {
			h2headersFrame.Priority.Weight = 99
		}

		session.SetStreamPriority(convertToProtocolPriority(h2headersFrame.Priority), protocol.StreamID(h2headersFrame.StreamID))
	}
...
func convertToProtocolPriority(httpPriority http2.PriorityParam) protocol.PriorityParam {
	return protocol.PriorityParam{
		StreamDep: protocol.StreamID(httpPriority.StreamDep),
		Exclusive: httpPriority.Exclusive,
		Weight:    httpPriority.Weight,
	}
}
...
func ListenAndServeQUIC(addr, certFile, keyFile string, handler http.Handler, scheduler string, streamScheduler string) error {
	server := &Server{
		Server: &http.Server{
			Addr:    addr,
			Handler: handler,
