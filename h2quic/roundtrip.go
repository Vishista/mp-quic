...
type QuicRoundTripper struct {
	...
	localAddr []string
	moreRemoteIps []string
}

func NewQuicRoundTripper(localAddr []string, moreRemoteIps []string, tlsConfig *tls.Config) *QuicRoundTripper {
	return &QuicRoundTripper{localAddr: localAddr, TLSClientConfig: tlsConfig, moreRemoteIps: moreRemoteIps}
}
...
func (r *QuicRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	return r.getClient(hostname, r.localAddr, r.moreRemoteIps).RoundTrip(req)
}

func (r *QuicRoundTripper) getClient(hostname string, localAddr []string, moreRemoteIps []string) http.RoundTripper {
	r.mutex.Lock()
	...
	if !ok {
		client = newClient(r.TLSClientConfig, hostname, &roundTripperOpts{DisableCompression: r.DisableCompression}, localAddr, moreRemoteIps)
