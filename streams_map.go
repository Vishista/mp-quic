...
type ByPrioWeight []*stream

func (stream ByPrioWeight) Len() int      { return len(stream) }
func (stream ByPrioWeight) Swap(i, j int) { stream[i], stream[j] = stream[j], stream[i] }
func (stream ByPrioWeight) Less(i, j int) bool {
	return stream[i].priority.Weight > stream[j].priority.Weight
}

//TODO CHECK priority in HTTP header frame .isZero  in	"golang.org/x/net/http2"
func PrioritizedStreamsIterate(m *streamsMap, fn streamLambda) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	...
	values := make([]*stream, 0, numStreams)
	for _, v := range m.streams {
		values = append(values, v)
	}
	sort.Sort(ByPrioWeight(values))

	for i := 0; i < numStreams; i++ {
		streamID := values[i].streamID
...
//This stream scheduler can cause to send packets with space left even thought there is data
func StrictPrioritizedStreamsIterate(m *streamsMap, fn streamLambda) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	...
	values := make([]*stream, 0, 0)
	for _, v := range m.streams {
		if v.streamID == protocol.StreamID(1) || v.streamID == protocol.StreamID(1) {
			continue
		}
		values = append(values, v)
	}
	...
	for i := 0; i < len(values); i++ {
		...
		if (!values[i].finishedWriting.Get()) || !cont {
			break
		}
	}
	return nil
...
//TODO StrictPrioritizedStreamsIterate sometimes has a deadlock and needs to be fixed
//Dont use it yet
func StrictPrioritizedStreamsIterateBLOCKING(m *streamsMap, fn streamLambda) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	...
	i := 0
	for i < numStreams {
		//always check for new data on control and header stream since they are most important
		for _, i := range []protocol.StreamID{1, 3} {
			cont, err := m.iterateFunc(i, fn)
			if err != nil && err != errMapAccess {
				return err
		...
		//finally check if the stream with highest priority has data to send
		streamID := values[i].streamID
		if streamID == 1 || streamID == 3 {
			i++
			continue
		...
		//Only send data of another stream if this stream is finished
		if values[i].finishedWriting.Get() {
			i++
		}
	}
	return nil
...
func HOBTestScheduler(m *streamsMap, fn streamLambda) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	...
	for _, i := range []protocol.StreamID{1, 3, 7} {
		cont, err := m.iterateFunc(i, fn)
		if err != nil && err != errMapAccess {
			return err
		}
	...
	for i := uint32(0); i < numStreams; i++ {
		...
		if streamID == 1 || streamID == 3 || streamID == 7 {
			continue
		}
...
func RoundRobinIterate(m *streamsMap, fn streamLambda) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *streamsMap) iterateFunc(streamID protocol.StreamID, fn streamLambda) (bool, error) {
	...
	if str.streamID != 1 && str.streamID != 3 {
		//fmt.Println("Stream", streamID, "has Priority", str.priority)
	}
	return fn(str)
}
