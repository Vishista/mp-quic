...
class StaticTopo(Topo):
    def build(self):
        ...
        self.addLink(s3, s4, bw=100, delay=10)
        self.addLink(s4, h2, bw=100, delay=10)
...
if __name__ == '__main__':
    ...
    h2.cmd('ifconfig h2-eth1 11.0.0.2')
    h1.cmd('ifconfig h1-eth0 10.0.0.1')
    h1.cmd('ifconfig h1-eth1 11.0.0.1')
    #for i in range(0, 2):
    #    h1.cmd('ifconfig h1-eth' + str(i) + ' 1' + str(i) + '.0.0.1')
    #    # heat up network to avoid initial high delays
    #    h1.cmd('ping 1' + str(i) + '.0.0.2 -c 4')

    # example for simple iperf traffic

    #h1.cmd('iperf -s -i 1 > server.log &')
    #h2.cmd('iperf -c 10.0.0.1 -t 10 > client.log')
    sleep(2)
    print "you might want to type xterm h1 h2 in the console and start wireshark in the correct context... enter exit to exit"
    CLI(net)

    h2.cmd('/usr/local/go/bin/go run ../example/server_main.go -v=0  -www ../example/web  &> server_mp.out &')
    # the server requires some startup time
    sleep(4)
    h1.cmd('/usr/local/go/bin/go run ../example/client/client_main.go -v=0 https://10.0.0.2:6121/testimage2.jpg &> client_mp.out')
    CLI(net)
    net.stop()
