...
if __name__ == '__main__':
    ...
    h2.cmd('/usr/local/go/bin/go run ../example/server_main.go  -sc='+scheduler+' -v=1 -www ../example/web  &> sp_server_'+scheduler+'.out &')
    # the server requires some startup time
    sleep(2)
    h1.cmd('/usr/local/go/bin/go run ../example/client/client_main.go -v=1 https://10.0.0.2:6121/test1Mb.db &> sp_client_'+scheduler+'.out')
    CLI(net)
    net.stop()
